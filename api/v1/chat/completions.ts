...
import { OpenAIChatRequest, OpenAIChatResponse, OpenAIChoice, OpenAIMessage } from '../../../lib/types';
...
// Arnaru models list (subset of available models)
...
function validateModel(model: string): string {
  ...
  return process.env.DEFAULT_MODEL || 'claude-fable-5';
}
  ...
  try {
    ...
    if (!arnaruResponse.ok) {
      ...
      return res.status(arnaruResponse.status).json({
        error: {
          message: `Arnaru API error: ${arnaruResponse.status} - ${errorText}`,
          type: 'api_error',
          code: arnaruResponse.status
        }
      });
    }
    ...
    if (parsed.hasError && !parsed.fullMessage) {
      return res.status(500).json({
        error: {
          message: parsed.errorMessage || 'Unknown error from Arnaru API',
          type: 'api_error'
        }
      });
    }

    const completion = createChatCompletion(requestId, model, parsed.fullMessage, parsed.newConversationId);
